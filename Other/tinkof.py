class DSU:
    def __init__(self, n):
        # Инициализируем родительский массив и ранг массива
        self.parent = list(range(n))
        self.rank = [0] * n

    def find(self, u):
        # Поиск корня для вершины u с путевой компрессией
        if self.parent[u] != u:
            self.parent[u] = self.find(self.parent[u])
        return self.parent[u]

    def union(self, u, v):
        # Объединяем множества, к которым принадлежат u и v
        root_u = self.find(u)
        root_v = self.find(v)
        if root_u != root_v:
...
def solve():
    ...
    for _ in range(k):
        ...
        v = int(data[index]) - 1
        index += 1
        queries.append((type_, u, v))

    # Создаем DSU для отслеживания компонент связности
    dsu = DSU(n)
    
    # Результаты запросов `ask`
    results = []
    ...
    for query in reversed(queries):
        ...
        if type_ == 'cut':
            # Добавляем обратно ребро в граф
            dsu.union(u, v)
        elif type_ == 'ask':
            # Проверяем, принадлежат ли u и v одной компоненте связности
